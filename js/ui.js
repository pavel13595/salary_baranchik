...
export function renderEmployeesTable() {
  ...
  if (!state.employees.length) { container.innerHTML = `<div class="empty">Немає співробітників. Додайте список.</div>`; return; }
  const grouped = []; const orderMap = new Map();
  state.employees.forEach(emp => { if (isDayOff(emp)) return; const g = emp.group || '_NO_GROUP_'; if (!orderMap.has(g)) orderMap.set(g, []); orderMap.get(g).push(emp); });
  const seen = new Set(); state.employees.forEach(emp => { if (isDayOff(emp)) return; const g = emp.group || '_NO_GROUP_'; if (!seen.has(g) && orderMap.has(g)) { grouped.push(g); seen.add(g); } });
  if (!grouped.length) { container.innerHTML = `<div class="empty">Немає записів (усі вихідні)</div>`; return; }
  const rows = [];
  rows.push(`<table class='payroll-table'><thead><tr><th>#</th><th>ПІБ</th><th>Посада</th><th>Ставка</th><th>Години</th><th>Продажі</th><th>Подарунки</th><th>Утримано</th><th>ЗП</th></tr></thead><tbody>`);
  ...
  container.querySelectorAll('td.editable').forEach(cell => { cell.addEventListener('blur', onCellEdit); cell.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); cell.blur(); } }); });
  if (!container.dataset.ctxBound) {
    container.addEventListener('contextmenu', e => {
      const tr = e.target.closest('tr[data-id]');
      if (!tr) return; e.preventDefault(); openEmployeeContextMenu(e, tr.dataset.id);
    });
...
export function openModal({ title, body, actions = [] }) {
  ...
  if (!layer) { layer = document.createElement('div'); layer.id = 'modalLayer'; layer.className = 'modal-layer'; document.body.appendChild(layer); }
  ...
  actions.forEach(a => { const btn = document.createElement('button'); btn.textContent = a.label; btn.className = a.class || ''; btn.onclick = () => a.onClick && a.onClick(); footer.appendChild(btn); });
  modal.querySelector('.close-btn').onclick = closeModal; layer.appendChild(modal); return modal;
}
export function closeModal() { const layer = document.getElementById('modalLayer'); if (layer) layer.classList.add('hidden'); }

export function importEmployeesFlow() {
  const ta = document.createElement('textarea'); ta.placeholder = 'Вставте список співробітників (з групами)';
  const helper = document.createElement('div'); helper.className = 'helper'; helper.textContent = 'Імʼя Прізвище\tПосада\t120 грн/год. Один стовпець = назва групи.';
...
export function editEmployeesFlow() {
  if (!state.employees.length) { showToast('Немає співробітників', 'error'); return; }
  const ta = document.createElement('textarea');
  const lines = []; let prev = '__INIT__';
  state.employees.forEach(e => { if (e.group && e.group !== prev) { lines.push(e.group); prev = e.group; } lines.push(`${e.name}\t${e.position}\t${e.rawRateStr}`); });
  ta.value = lines.join('\n');
  const helper = document.createElement('div'); helper.className = 'helper'; helper.textContent = 'Редагуйте список. Рядок без табів = група. Зберігаємо години/продажі по порядку.';
  const body = document.createElement('div'); body.append(ta, helper);
...
export function importHoursFlow() {
  if (!state.employees.length) { showToast('Немає співробітників', 'error'); return; }
  const ta = document.createElement('textarea'); ta.placeholder = '10:00-21:00 кожен рядок';
  const helper = document.createElement('div'); helper.className = 'helper'; helper.textContent = 'Порожні рядки ігноруються. "в" = пропуск. "вихідна" / "вибув" = службова позначка.';
  const body = document.createElement('div'); body.append(ta, helper);
...
function promptSalesIfNeeded() {
  const targets = state.employees.filter(e => (e.rateType === 'waiter' || e.rateType === 'hostess') && e.hoursMinutes > 0);
  if (!targets.length) return;
  const body = document.createElement('div'); body.innerHTML = '<div class="helper">Введіть продажі / подарунки</div>';
...
export function bindGlobalEvents() {
  ...
  if (dateInput) { dateInput.value = state.settings.reportDate; dateInput.addEventListener('change', () => { state.settings.reportDate = dateInput.value; persist(); }); }
}
...
export function openEmployeeContextMenu(e, id) {
  ...
  if (rect.right > vpW) { x = Math.max(4, x - (rect.right - vpW)); }
  if (rect.bottom > vpH) { y = Math.max(4, y - (rect.bottom - vpH)); }
  menu.style.left = x + 'px'; menu.style.top = y + 'px'; menu.style.visibility = 'visible';
  function outside(ev) { if (!menu.contains(ev.target)) { menu.remove(); document.removeEventListener('click', outside); } }
